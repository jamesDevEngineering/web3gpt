...
export const getChainDetails = (viemChain: Chain): ChainDetails => {
	...
	return {
		...
		explorerUrl:
			blockscoutUrl ||
			chainDetails.explorerUrl ||
			viemChain.blockExplorers?.default.url ||
			"",
		explorerApiUrl: blockscoutUrl
			? buildApiUrl(blockscoutUrl)
			: chainDetails.explorerApiUrl ||
			  viemChain.blockExplorers?.default.apiUrl ||
			  "",
		explorerApiKey: blockscoutUrl
			? BLOCKSCOUT_API_KEY
			: chainDetails.explorerApiKey,
	};
};
	...
	return createConfig({
		...
		transports: Object.fromEntries(
			supportedChains.map((chain) => [
				[chain.id],
				http(CHAIN_DETAILS[chain.id].rpcUrl),
			])
		),
		ssr: true,
